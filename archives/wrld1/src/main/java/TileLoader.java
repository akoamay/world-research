...
//            _map = null;
    ...
    public FloatArrays genFloatArray( byte data[][] ){
       ...
       int y = data[0].length;
        for (int i = 0; i < y; i++){
            for (int j = 0; j < x; j++){
                    ...
                    */
                    ...
                    float  o = (x - 1) / 2.0f;
                    vtxArraylist.addAll( getFacetArray(BOTTOM,j-o,i-o,0) );
        ...
        if ( vtxArraylist.size() != 0) System.out.println( "val.size=" + vtxArraylist.size() + " cc=" + cc );
        ...
        return new FloatArrays(vtxFloatArray,nrmFloatArray,colFloatArray);
    }
}